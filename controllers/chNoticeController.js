...
var getCountNewNoticeOnChannel = function (request, response) {
...
var SelectMaxSequenceNumOnChannel = function (request, callback) {
    
    if (request.param("N_SEQ") != "create") {
        callback("0");
    } else {
        logger.info('Path change: /SelectMaxSequenceNumOnChannel');
        var query = util.format('SELECT ifnull(MAX(CAST(N_SEQ as SIGNED integer)),0) MAX_SEQ FROM TB_TN_SERVICE WHERE P_CUST_CTN = \'%s\' AND P_INSERT_DATE = \'%s\''
...
var InsertNoticeDataOnChannel = function (request, response) {
    
    SelectMaxSequenceNumOnChannel(request, function(MAX_SEQ) {
        ...
                    logger.info('DB Success:');
                    if (Object.keys(results).length > 0) {
                        ...
                        InsertNotice(request, response, results[0].SEQ, MAX_SEQ);
                    } else {
        ...
            });
            ...
            InsertNotice(request, response, request.param('P_CUST_CTN'), MAX_SEQ)
        }
    });
};

var InsertNotice = function (request, response, key ,MAX_SEQ) {
...
var  SelectNoticeDataOnChannel = function (request, response) {
...
var  UpdateNoticeDataOnChannel = function (request, response) {
...
var  UpdateSystemNoticeChangeStatus = function (request, response) {
...
// var  UpdateSystemNoticeResend = function (dbConn, request, response) {
//     var query = "UPDATE TB_TN_SERVICE " +
//     "SET N_READ_FLAG = '0' " +
...
var  getSystemNoticeContent = function (request, response) {
...
var getServiceNoticeContent = function (request, response) {
...
module.exports = {
    getCountNewNoticeOnChannel,
    InsertNoticeDataOnChannel,
    SelectNoticeDataOnChannel,
    UpdateNoticeDataOnChannel,
    UpdateSystemNoticeChangeStatus,
    // UpdateSystemNoticeResend,
    getSystemNoticeContent,
    getServiceNoticeContent    
};
