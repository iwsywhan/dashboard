    ...
    var JWT = decodeJWT(req, res, function(result, token) {
        if (result) {
            ...
            var query = util.format('SELECT C_VALUE as STORAGE' +
                ...
                ' FROM TB_TERMINAL_IMAGE_TRANS WHERE CODE_03 = \'%s\'', token.code_03, token.code_03);
...
var mainPage = function(req, res) {
    logger.info('Path move : /service/mainPage');

    if (req.session.drone !== 'Y') {

        var JWT = decodeJWT(req, res, function(result, token) {
            if (result) {
                fs.readFile('html/service_status.html', 'utf8', function(error, data) {
                    var storage_size;
                    var query = util.format('SELECT C_VALUE as STORAGE' +
                        ' FROM TB_COMMON WHERE C_NAME = concat(\'STORAGE_\', (SELECT SV_OP_SV_S FROM TB_CUSTOMER WHERE CUSTOMER_CODE = \'%s\'))' +
                        ' UNION' +
...
var droneStatus = function(req, res) {
    ...
    var JWT = decodeJWT(req, res, function(result, token) {
        if (result) {
            var code1 = token.code_01;
            var code2 = token.code_02;
            var code3 = token.code_03;
            var s_userid = token.id;
            var userlevel = token.userlv;
            
            var query;
            // query = "SELECT ";
            // query += "CUST_CTN, ";
            // query += " CTN_DEVICE, INSERT_DATE, CUST_NM, CUST_DEPT_NM, STATUS,SVC_TYPE ";
            // query += " ,DEV_TYPE, CAMERA_TYPE, VCODEC, FPS, WIDTH, HEIGHT, BIT_RATE, LOGIN_DATE, thumimg ";
            // query += " FROM TB_TERMINAL_IMAGE_TRANS A ";
            // query += " LEFT JOIN  ";
            // query += "        ( "; 
            // query += "        SELECT P_CUST_CTN, P_INSERT_DATE, SUBSTRING_INDEX(IMG_FILE_NM, '/', -2) thumimg ";
            // query += "        FROM TB_THUMBIMG_ANA_HISTORY  ";
            // query += "        ORDER BY INSERT_DATE DESC LIMIT 0, 1 ";
            // query += "        ) B ";
            // query += " ON A.CUST_CTN = B.P_CUST_CTN AND A.INSERT_DATE AND B.P_INSERT_DATE ";
...
var dronesStatus = function(req, res) {
    ...
    var JWT = decodeJWT(req, res, function(result, token) {
        if (result) {
            ...
            var query;
            // query = "SELECT ";
            // query += "SUBSTRING_INDEX(CUST_CTN, '__', 1) CUST_CTN, CTN_DEVICE, IFNULL(STATUS, '3') STATUS, SVC_TYPE, DEV_TYPE ";
            // query += " FROM (SELECT DEV_KEY, DEPT_CODE_03 FROM TB_DRON_SETUP_INFO) A ";
            // query += " LEFT JOIN TB_TERMINAL_IMAGE_TRANS B ";
            // query += " ON A.DEV_KEY = B.CUST_CTN ";
            // query += " WHERE STATUS < '3' ";
            // if (userlevel == 1) {
            //     query += ' AND CODE_03=\'' + code3 + '\' ';
            // } else {
            //     query += ' AND CODE_01=\'' + code1 + '\' AND CODE_02=\'' + code2 + '\' AND CODE_03=\'' + code3 + '\' ';
            // }
            // query += " GROUP BY SUBSTRING_INDEX(CUST_CTN, '__', 1)";
